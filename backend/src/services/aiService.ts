...
class AiService {
  ...
  public async analyzeLogs(logs: string) {
    try {
      ...
      return {
        summary:
          'Container logs indicate an authorization verify error followed by a failure to connect to an external Docker Registry.',
        root_cause:
          'The JWT signature verification failed due to secret mismatch. Additionally, the Docker daemon could not resolve the ECR DNS query, pointing to a network gateway or DNS server configuration issue in the cluster.',
        recommendations:
          '1. Verify that JWT_SECRET matches across your backend services.\n2. Verify CoreDNS configurations in the cluster to ensure external registry names resolve properly.',
      };
    }
  }
  ...
  public async chat(message: string, history: any[]) {
    try {
      ...
      let reply =
        'I am the Deploymate AI DevOps Co-Pilot. I can diagnose failed pipeline logs, assess manifest security risks, generate CI/CD code templates, and explain cluster operations.';
      if (msg.includes('kubernetes') || msg.includes('k8s')) {
        reply =
          "Deploymate visual explorer supports Namespaces, Pods, Deployments, and Services. You can rolling restart or roll back any deployment to its previous stable revision by clicking 'Rollback'.";
      } else if (msg.includes('pipeline') || msg.includes('workflow')) {
        reply =
          'Deploymate features an 8-stage pipeline runner (Source, Build, Test, Code Quality, Security Scan, Docker Build, Image Push, Deploy) which streams runtime execution logs over WebSockets.';
      }
